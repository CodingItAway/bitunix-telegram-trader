...
/**
 * Generic save to Google Drive
 * @param {string} fileName - Name of the file (e.g., 'config.json')
 * @param {string} content - JSON string content to save
 */
async function saveToDrive(fileName, content) {
  ...
  try {
    ...
    const listRes = await drive.files.list({
      q: `name='${fileName}' and '${FOLDER_ID}' in parents and mimeType='application/json' and trashed=false`,
      fields: 'files(id)',
      supportsAllDrives: true
    });
    ...
    console.error(`[GDrive] Failed to save ${fileName}:`, err.message);
    throw err;
  }
}

/**
 * Generic load from Google Drive
 * @param {string} fileName - Name of the file to load
 * @returns {string|null} File content as string, or null if not found
 */
async function loadFromDrive(fileName) {
  if (!drive && !(await authGoogle())) {
    console.warn('[GDrive] Not authenticated');
    return null;
  }
  ...
  try {
    ...
    if (listRes.data.files.length === 0) {
      console.log(`[GDrive] ${fileName} not found`);
      return null;
    }
    ...
    console.log(`[GDrive] Loaded ${fileName}`);
    return res.data; // returns string content
  } catch (err) {
    console.error(`[GDrive] Failed to load ${fileName}:`, err.message);
    return null;
  }
}

module.exports = { loadPositions, savePositions, loadHistory, saveHistory, saveToDrive, loadFromDrive };
